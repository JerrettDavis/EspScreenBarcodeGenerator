...
- Add a new transport (Bluetooth Classic RFCOMM, a cross-platform BLE connector for Linux/macOS) by implementing a new connector/endpoint against `lib/EspLinkCore`'s ports (`ControlProtocolEngine`, the `Envelope`/`HopFrame`/`Cobs` codecs, `IControlResponseSink`) — see [`docs/PROTOCOL_V2.md`](PROTOCOL_V2.md)'s extension guide and "Next PRs" section for the concrete steps and known gaps (including the .NET `TransportSelector` fallback-policy parity gap). Three worked examples of this pattern now exist: the ESP-NOW compatibility endpoint (`src/EspNowEndpoint.cpp`, single-board radio-init validated only), the BLE GATT endpoint (`src/BleGattEndpoint.cpp` + Windows `dotnet/src/EspBarcode.Client.Ble`, hardware-validated end-to-end — real Windows PC talking to the flashed display over Bluetooth LE), and the Wi-Fi Direct TCP endpoint (`src/WifiDirectTcpEndpoint.cpp` + Windows `dotnet/src/EspBarcode.Client.WifiDirect`, credential-provisioning-over-BLE validated end-to-end, radio join blocked on this dev machine by an Intel Wi-Fi driver limitation — see PROTOCOL_V2.md §10 for the exact evidence). A USB↔ESP-NOW gateway is also implemented (`src/GatewayRelay.cpp`, a runtime mode of this same firmware rather than a separate board image — see PROTOCOL_V2.md §10) with a matching Windows connector (`dotnet/src/EspBarcode.Client/TransportV2/GatewaySerialConnector.cs`). BLE on Linux/macOS is still future work.
