...
**On-device gateway monitoring screen**: once a board switches into gateway mode, its touchscreen Home layout replaces the normal barcode editor with a "GATEWAY MODE" banner and a "VIEW GATEWAY STATS" button (`BarcodeApplication::drawGatewayHomeBanner`, `src/BarcodeApplication.cpp`) — editing a barcode locally has no purpose once the board is a pure relay. Tapping it opens a live stats screen (`View::Gateway`) showing USB host connection status (up/lost, with age since last byte), the count and MAC/last-seen age of distinct ESP-NOW senders seen this session, and the per-direction relayed-message counters, refreshed roughly once a second while the screen is open. Gateway mode has no pairing handshake, so there's no protocol concept of a "negotiated client" or a heartbeat message to surface — the screen approximates both from recency: a peer is listed as soon as any frame from its MAC is accepted, and its "heartbeat" is simply how long ago that last frame arrived. This bookkeeping lives in a new portable, natively-tested helper, `lib/EspLinkCore/src/GatewayStats.h`/`.cpp` (`tests/esplink_gateway_stats_tests.cpp`) — a fixed 8-peer least-recently-seen cache plus a host-activity timestamp — that `GatewayRelay` feeds from its existing RX/TX paths and exposes via `GatewayRelay::stats()`; `main.cpp` pushes a fresh snapshot into `BarcodeApplication::updateGatewayStats()` every loop iteration while gateway mode is active.

Since the gateway/upgrade handshakes are one-way switches with no software revert path, the only in-device way back to the normal ESP Barcode Lab screen is a restart: both the Home banner and the gateway stats screen carry a "RESTART DEVICE"/"RESTART" button (`BarcodeApplication::rebootDevice`) that runs the same `Serial.flush(); delay(100); ESP.restart();` sequence as the existing `device.reboot` protocol command (`EspIdfDeviceControl::reboot`) — after the restart the board boots fresh into normal client mode, same as a physical power cycle.

### Gateway ↔ client discovery ping/pong (implemented)

Gateway mode's "no pairing handshake" (§10 above) meant a gateway could only ever learn about a client by already relaying real traffic to it, and a client had zero notion of whether a gateway existed nearby at all. This adds a lightweight, symmetric discovery exchange on top of the previously-reserved-but-unimplemented `ServiceId::Gateway = 7` namespace (§4's `gateway.*` commands, first named in the multi-transport design plan and 0% implemented until now):

- **Wire messages** (`gateway.link.ping` / `gateway.link.pong`, both `MessageKind::Event`, `ServiceId::Gateway`, ESP-NOW broadcast, JSON body `{role: "gateway"|"client", deviceId, firmware, ts, echoTs?}`): either role may broadcast a ping; whoever receives one addressed to the *other* role replies with a pong that echoes the ping's `ts` back as `echoTs`, letting the sender compute RTT locally. There is still no pairing/trust step — this is discovery-only, matching the existing broadcast-everything trust model.
- **Gateway side** (`GatewayRelay`): broadcasts a `gateway.link.ping` every ~2s (`kDiscoveryPingIntervalMs`) and immediately replies to any `gateway.link.ping{role:"client"}` it receives; every `gateway.link.pong{role:"client"}` reply updates the peer's entry in the existing `GatewayStats` peer table (extended with `everPinged`/`lastRttMs`/`deviceId` alongside the pre-existing `everRelayed`/`lastSeenMs`, so one peer table now covers both "carried real relayed traffic" and "answered a discovery probe" — a peer can be either, both, or (briefly) neither).
- **Client side** (`EspNowEndpoint`): tracked via a new portable `esplink::GatewayLinkInfo`/`IGatewayLinkStatusSource` pair (`lib/EspLinkCore/src/ApplicationPorts.h`) so `ControlProtocolEngine` can surface it through the ordinary `status` command's new optional `gateway_link` field without depending on any ESP-NOW/Arduino header itself — `ControlProtocolEngine::setGatewayLinkStatusSource()` is an optional post-construction wire-up (`main.cpp`), not a constructor dependency, so host-side unit tests are unaffected. A client broadcasts its own `gateway.link.ping{role:"client"}` on a backoff (every 3s while it hasn't seen a gateway recently, every 8s once it has, to confirm liveness) and answers any gateway ping immediately.
- **Host-facing gateway-local commands** (`gateway.peers.list`, `gateway.ping.now`): a deliberate, narrow crack in `GatewayRelay`'s protocol-blind purity — it now decodes just enough of a USB-origin envelope to check `serviceId`, and a `ServiceId::Gateway` message is answered *locally* (peer list snapshot / immediate discovery broadcast) instead of being relayed across ESP-NOW, since these queries are about the gateway board's own state, not the far-side relayed display's. Every other `serviceId` still relays exactly as before, completely unaware this branch exists.
- **UI**: the Gateway page (Blazor) gained "Ping for Clients"/"Refresh Peers" plus a per-peer MAC/device-id/RTT/last-seen table (`GatewayLinkClient.ListPeersAsync`/`PingNowAsync`); the Devices page shows a plain client's own gateway-link status badge. The on-device TFT gateway stats screen gained a **PING** button and per-peer RTT; a plain client's Settings screen gained a compact "ESP-NOW Gateway: searching…/connected" row, reusing the existing colored-dot pill idiom.

**Hardware-validated (partial):** with one board flashed and put into gateway mode, a real host round-tripped `gateway.peers.list` (`{"peers":[]}`, correctly empty with no second board present) and `gateway.ping.now` (`{"ok":true}`, broadcasts without crashing) over real USB↔hop-frame↔envelope framing, and `status` on a plain (non-gateway) client board returned the new `gateway_link` field correctly (`{"connected":false,"age_ms":0,"rtt_ms":0}`) — confirming the new local-command interception and the portable status-source wiring both work on real silicon. The two-board discovery round trip itself (a gateway's ping actually reaching a second real board and its pong updating the peer table with a real RTT) is exercised only by the BDD suite's byte-accurate `fakeSerial.js` emulator this session, since only one ESP32 board with a compatible chip target (`esp32dev`) was available; a second, unrelated ESP32-S3 device present on this machine was left untouched (wrong chip for this project's `platformio.ini` environment, and not a board this project owns).
