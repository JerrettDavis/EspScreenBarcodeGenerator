...
### Wi-Fi Direct legacy GO + TCP (still not implemented)

Implement the same connector seam over a Windows-created group-owner TCP listener and an ESP32 Wi-Fi station endpoint. Reuse the `TcpStandard`/`TcpLarge` `CarrierProfileId` values already reserved in `lib/EspLinkCore/src/ConnectivityTypes.h` (§5) rather than adding new ones. TCP is a byte stream, so its connector belongs with `SerialLinkConnection`/`EspLinkLinkSession`'s COBS-stream model (§8, extension guide), not the datagram model `EspLinkDatagramLinkSession` uses (see BLE GATT section below) — a length-prefix framing choice is already specified (§8.6) and does not need COBS. Deliberately deprioritized behind BLE this session: it requires a Windows `WiFiDirectAdvertisementPublisher` (legacy group-owner) integration that could not be built and hardware-validated blind in the same session as BLE without risking shipping two unvalidated transports instead of one solid one.
